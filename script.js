...
const history_info = [
    ...
    {
        year: '1970 - 1980',
        text: 'В период с 1970 по 1980 в регионе активно развивалась промышленность и создавались научно-производственных объединений в сферах приборостроения и электротехники. Кроме того 19 декабря 1976 года был произведен запуска первого энергоблока Курской АЭС.',
...
function renderHistoryFeed() {
    ...
    history_info.forEach((item, index) => {
        ...
        const total_img = item.img ? `<img load="lazy" src="${item.img}" class="container_history_right_block_img">` : ''
    ...
    placeholders.forEach(placeholder => {
        const html_content = `
            <section class="container_beginning_chapter">
                <img load="lazy" class="container_beginning_chapter_img" src="${placeholder.dataset.image}" alt="Изображение начала главы">
...
class Carousel {
    ...
    init() {
        this.container.innerHTML = `
            <img load="lazy" class="info_card_img" src="${this.slides[this.currentSlide].image}" alt="${this.slides[this.currentSlide].title}">
...
const folk_crafts = [
    {
        ...
        text: 'Курская область славится своим народными промыслами: гончарное ремесло, резьюа по дереву, вышивка, Суджанское ковроткачество, но самым запоминающимя среди туристов и местных жителей считается - Кожлянская глиняная игрушка.',
        link: 'https://welcomekursk.ru/overviews/140/kurskie-promysly'
    },
    {
        image: './img/drama_theater.jpg',
        title: 'Учреждения культуры',
        text: 'Культурные учреждения Курской области: музеи, театры, филармонии, дома культуры -  отражают ее многовековую историю, культуру и уникальный быт. Все они играют важную роль в жизни людей: сохраняют память, развавают кругозор и обучают подрастающее поколение.',
        link: 'https://www.culture.ru/traditions/doma-kultury-i-kluby/location-kurskaya-oblast'
    },
    {
        image: './img/religion.webp',
        title: 'Религия',
        text: 'Курская земля во все времена славилась большим влечением к религиозным традициям. В связи с чем на этой земле было построено большое количесттво религиозных учреждений: Коренная Рождество-Богородицкая пустынь, Знаменский мужской монастырь и т.д.',
        link: 'https://ru.wikipedia.org/wiki/Коренная_пустынь'
    },
    {
        image: './img/festivals_holidays.jpg',
        title: 'Фестивали и праздники',
        text: 'По сей день в Курской области уделяется особое внимание фестивалям и традициям, отражабщим ее культуру, быт и исторические традиции. Самыми популярными из которых являются: Курская Коренская ярмарка (июнь), День города Курска (25 сентября), Военная реконструкция «Курская дуга» (июль–август).',
    ...
    },
    {
        ...
        text: 'Традиционная кухня Курской области это просая, но вкусная еде, приготовленная из ржи, гречихи, картофеля, молока, грибов, ягод и мяса. Влияние украинской и южнорусской кухни придало ей особый контингент.',
...
const ecology_cards = [
    ...
    },
    {
        ...
        title: 'Рельеф',
        text: 'Территория Курской области расположена на юго-западных склонах Среднерусской возвышенности. Характеризуется наличием древних и современных форм линейной эрозии — густой сети речных долин, оврагов и балок, расчленивших водораздельные поверхности, что слегка всхолмлённый равнинный рельеф.',
...
const nature_cards = [
    {
        ...
        text: 'Дендропарк создавался на научной основе в соответствии с рекомендациям ученых-ботаников Брянской и Воронежской областей. Сегодня в парке собраны растения из разных уголков земного шара. На территории в три гектара высажены более 3500 редких растений. Для удобства парк разделен на 34 сектора.',
    ...
    },
    {
        ...
        text: 'Статус памятника природы регионального значения был официально утвержден в 2013 году. Особенностью озера является его плавучий остров, состоящий из растений. На территории водоёма обнаружено 187 видов сосудистых растений из которых 11 включены в Красную книгу Курской области.',
